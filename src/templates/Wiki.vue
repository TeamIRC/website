...
const emits = defineEmits<{
	(event: "modified", template: string, content: WikiSection): void
}>();
...
watch(
    ...
    () => {
		if (list.value.items.toString() !== props.content.items.toString())
			emits("modified", "Wiki", list.value);
	}
);
</script>
...
<template>
    <ListBase :edit="edit" :items="content.items" :empty-item="{
        ...
        <template #editor="{ index }">
			<input type="text" v-model="list.items[index].thumbnail" placeholder="Image (ex: wiki-article.webp)" />
			<input type="text" v-model="list.items[index].title" placeholder="Titre de l'article Wiki" />
        ...
        <template #item="{ item, index }">
            <router-link class="item-container card v-center" :to="'/wiki/' + index">
                ...
                <div class="description">
                    ...
                    <div v-if="item.internalLinks && item.internalLinks.length > 0" class="related-links">
                        <SVGIcon name="link" />
                        <span>{{ item.internalLinks.length }} lien{{ item.internalLinks.length > 1 ? 's' : '' }} interne{{ item.internalLinks.length > 1 ? 's' : '' }}</span>
                    </div>
                </div>
			</router-link>
        </template>
